...
import { sql, type DBInvite, type DBGroup, type DBGroupUser } from "~/server/db";
...
export const invitesRouter = createTRPCRouter({
    ...
    send: protectedProcedure.input(
        ...
        try {
            ...
            const inviteInsertResult: DBInvite[] = await sql`
                INSERT INTO invites (senderId, recipient_id, group_id, status)
    ...
    accept: protectedProcedure.input(
        ...
        try {
            ...
            const validate: DBInvite[] = await sql`
                SELECT * 
                FROM invites
                WHERE groupId = ${groupId} AND senderId = ${senderId} AND recipient_id = ${ctx.user.id}
            `;
    ...
    withdraw: protectedProcedure.input(
        ...
        try {
            ...
            const withdrawInvite: boolean[] = await sql`
                DELETE FROM invites
                WHERE groupId = ${groupId} AND senderId = ${ctx.user.id} AND recipient_id = ${recipientId}
                RETURNING true
            `;
     ...
     * @param senderId The sender of the invite
     */
    decline: protectedProcedure.input(
        z.object({
            groupId: z.number(),
            senderId: z.number()
        })
     ...
     */
    getOutboundInvites: protectedProcedure.query(async ({ctx}): Promise<DBInvite[]> => {
        try {
     ...
     */
    getInboundInvites: protectedProcedure.query(async ({ctx}): Promise<DBInvite[]> => {
        try {
