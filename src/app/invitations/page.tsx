            ...
            {myInvites && myInvites.length > 0 ? (
              <CardContent className="flex flex-1 flex-col space-y-4 pb-12 sm:space-y-6 sm:pb-16">
                {myInvites.map((value, key) => (
                  <div
                    key={key}
                    className="flex w-full justify-between rounded-full bg-white p-4"
                  >
                    <p className="flex w-1/3 items-center justify-start">
                      Group:{" "}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        acceptInvite(value.group_id, value.sender_id)
                      }
                    >
                      Accept
                    </Button>
                    <Button
                      variant="danger"
                      className="ml-2 h-10 rounded-full px-6"
                      onClick={() =>
                        declineInvite(value.group_id, value.sender_id)
                      }
                    >
                      Decline
                    </Button>
